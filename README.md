...
| 8 | `11_earth_track.py`, `12_hss_timing.py`, `13_polarity_agreement.py`, `14_expansion_proxy.py` | cross-height secondary analyses (Earth-connected track coverage, high-speed-stream timing, IMF polarity agreement, radial expansion proxy) |
| 9 | `15_final_results.py` | locked science sample, pooled and per-CR correlations, leave-one-out table, null tests in `outputs/tables/final/` and `outputs/results/final/` |
